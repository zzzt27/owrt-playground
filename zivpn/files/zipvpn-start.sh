...
EXPOSE_ADDR="$(uci -q get zipvpn.main.expose_addr 2>/dev/null)"; EXPOSE_ADDR="${EXPOSE_ADDR:-0.0.0.0}"
EXPOSE_PORT="$(uci -q get zipvpn.main.expose_port 2>/dev/null)"; EXPOSE_PORT="${EXPOSE_PORT:-7777}"
...
log "Obfs Key    : $Z_OBFS"
log "Listen      : $EXPOSE_ADDR:$EXPOSE_PORT"
...
CFG="$CFG,\"socks5\":{\"listen\":\"$EXPOSE_ADDR:$EXPOSE_PORT\"}"
CFG="$CFG,\"insecure\":true,\"recvwindowconn\":$RECV_CONN,\"recvwindow\":$RECV_WIN,\"disable_mtu_discovery\":$DISABLE_MTU,\"resolver\":\"8.8.8.8:53\",\"down_mbps\":$DOWN_MBPS,\"up_mbps\":$UP_MBPS}"
...
for i in $(seq 1 15); do
    ...
    if netstat -tunlp 2>/dev/null | grep -q ":$EXPOSE_PORT"; then
        log ""
        log "=== ZipVPN READY ==="
        log "SOCKS5 on $EXPOSE_ADDR:$EXPOSE_PORT"
        log ""
        log "Mihomo config:"
        log "  - name: zipvpn"
        log "    type: socks5"
